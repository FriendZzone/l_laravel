    ...
    <form action="{{ route('users.postEdit', ['id' => $id]) }}" method="POST" class="d-flex flex-column">
            ...
            <select id="group-select" class="form-control" name="group_id">
                ...
                @if (!empty($allGroups))
                    @foreach ($allGroups as $groupItem)
                        <option value="{{ $groupItem->id }}" {{ (old('group_id') == $groupItem->id || $userDetail->group_id == $groupItem->id) ? 'selected' : false }}>
                            {{ $groupItem->name }}
                        </option>
                    @endforeach
                @endif
        ...
        <div class="mt-2">
            ...
            <select id="status-select" class="form-control" name="status">
                <option value="0" {{ (old('status') == 0  || $userDetail->group_id == 0) ? 'selected' : false }}>Inactive</option>
                <option value="1" {{ (old('status') == 1  || $userDetail->group_id == 1) ? 'selected' : false }}>Active</option>
            </select>
            @error('status')
                <span style="color:red;">{{ $message }}</span>
            @enderror
